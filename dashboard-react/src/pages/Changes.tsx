...
const chip = "inline-flex w-fit items-center rounded-md border px-2 py-0.5 text-xs font-medium";
  ...
  return (
    <div className="page-container max-w-[960px]">
      <header>
        ...
        <p className="page-description">Detected application, document, decision, and hash changes across tracked councils.</p>
      </header>
              ...
              <div key={log.run_id} className="flex flex-col gap-2">
                ...
                <div className="card-surface divide-y divide-border">
                  {log.changes?.new_applications?.map((app) => (
                    <div key={app.application_id || app.council || "new-application"} className="grid gap-2 px-4 py-3 sm:grid-cols-[150px_1fr] sm:gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 line-clamp-2 text-sm leading-6 text-muted-foreground">{app.proposal}</p>
                      </div>
                    </div>
                  ))}

                  {log.changes?.updated_decisions?.map((update) => (
                    <div key={update.application_id || update.new_decision || "decision-update"} className="grid gap-2 px-4 py-3 sm:grid-cols-[150px_1fr] sm:gap-3">
                  ...
                  {log.changes?.new_documents?.map((doc, i: number) => (
                    <div key={i} className="grid gap-2 px-4 py-3 sm:grid-cols-[150px_1fr] sm:gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 break-words text-sm leading-6 text-muted-foreground">{doc.filename || doc.type || doc.url}</p>
                      </div>
                    </div>
                  ))}
                  ...
                  {log.changes?.changed_hashes?.map((hashObj, i: number) => (
                    <div key={i} className="grid gap-2 px-4 py-3 sm:grid-cols-[150px_1fr] sm:gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 break-words text-sm leading-6 text-muted-foreground">{hashObj.filename} hash changed unexpectedly.</p>
                      </div>
                    </div>
                  ))}
